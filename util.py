...
class ASCollection():
    ...
    def restore_from_fs(self, path):
        """ Restore from file system.
        """

        all_files = [f.path for f in os.scandir(path) if f.is_file()]
        col_file = [f for f in all_files if 'collection' in f][0]
        page_files = [f for f in all_files if 'page' in f]

        with open(col_file) as f:
            self.col = json.load(f, object_pairs_hook=OrderedDict)
        for pf in page_files:
            page = ASCollectionPage(None, pf)
            with open(pf) as f:
                page.from_json(f.read())
            self.add(page)
